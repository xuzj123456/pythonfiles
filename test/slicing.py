...
class Point:  #点的信息（x,y,z）
    def __init__(self, x_, y_, z_): #坐标
        self.x = x_
        self.y = y_
        self.z = z_

    def dotProduct(self, p): #标量积
        return self.x*p.x + self.y*p.y + self.z*p.z

    def normalize(self): #求距离？
        return math.sqrt(self.x**2 + self.y**2 + self.z**2)

    def toString(self): #坐标值表示
        return "Point("+str(self.x)+","+str(self.y)+","+str(self.z)+")"
    def equals(self, p2):  #判断重合？
        if close(self.x,p2.x) and close(self.y,p2.y) and close(self.z,p2.z):
            return True
        else:
            return False

def pointInLine(p, line):  #判断是否在线上
...
class Line: #线的信息（两个顶点坐标）
    ...
    def reverse(self): #线段方向反转
        x_ = copy.copy(self.p0.x)  #浅复制
...
#for floating point comparison用于浮点比较
def close(f1,f2):  #在一定精度下判断两点是否重合
    comp = (max(f1,f2) - min(f1,f2))
    return (comp > -delta) and (comp < delta)

def lineEqual(L1,L2): #两条线是否重合（方向可以相反）
...
def triangleEqual(T1,T2): #三角形是否重合
...
def fileToTriangles(filename): #文件转化为三角面片。问题：如何转化？
    with open(filename, 'r') as f:
        next(f) #next() 方法在文件使用迭代器时会使用到，在循环中，next()方法会在每次循环中调用，该方法返回文件的下一行，如果到达结尾(EOF),则触发 StopIteration
        counter = 0
        triangles = list()
        points = list()
        for line in f:
...
def intersectSlice(line, plane):  #线段平面求交？
...
# False otherwise
def aboveTriangle(point,triangle):  #判断点是否在三角形正上方，问题：sign是干啥用的？
...
def findBoundaries(triangles):   #返回三角形列表中的最高最低z值，问题：为啥要设定是+-500
    bottomZ = 500
    topZ = -500
...
# is a bottom or top
def separateSlices(triangles, layerThickness): #给三角形列表和层厚，返回各切片线段？
    bounds = findBoundaries(triangles)
    numSlices = int((bounds[1]-bounds[0])/layerThickness) #求层数
    slices = [bounds[0]+z*layerThickness for z in range(0, numSlices+1)] #每层Z值？
    segments = list()
    for s in slices:
        currentSegment = list()
...
def intersection(L1,L2):  #判断同平面两线段是否相交，若相交，交点在哪？
...
def infill(perimeter,percent):  #给定线段列表和填充率，返回填充路径线段？
...
def findNextPoint(point, lines): #给定线段列表和起点，找和起点连接的线
...
def cleanPerimeter(s): #做优化用，给定一个切片的线段列表，返回去除重复线段和内部线段的列表（去除内部线段的没有启用）
    #for line in s:
    ...
    while i < len(setPerimeter):
        ...
        while j < len(setPerimeter):
            if lineEqual(setPerimeter[i],setPerimeter[j]):#去除重复线段
...
# will also generate a skirt if offset is greater than 1 and number of outlines = 1 ？没懂
def brim(base, numOutlines, offset):
    # compute centroid of polygonal manifold 计算多边形质心
    cx = 1
    cy = 1
    area = 1
    for line in base:
...
def raft(slices, numLayers, offset, infill, layerThickness): #生成位于物体之下的raft，可以不用？
...
def downward(triangles):# 给三角形列表，返回向下的三角形列表（法向量z小于0）
    trianglesDown = list()
    for triangle in triangles:
...
def supportNeeded(triangle, triangles, bottomZ):#给定一个向下的三角形和一个三角形列表 有阻挡->false 无阻挡->ture
    ...
    for tri in triangles:
        if (aboveTriangle(triangle.p0, tri)  #向下的三角形的点在三角形之上，说明有阻挡
...
def generateSupportShape(triangle, bottomZ): #给定一个需要支撑和最小Z值的三角形，返回在该三角形下形成支撑形状所需的三角形列表 （检测需要加支撑的位置并加支撑）
...
def generateSupports(triangles, layerThickness):#支撑，问题：为啥要给向下的三角形加支撑
...
def writeGcode(slices,filename):#给定切片和线段列表，生成G代码
...
def main():
    ...
    infillPercent = float(sys.argv[3]) ！！！！为简便处理暂不使用命令行
    """
    filename = 'cube.stl'
    layerThickness = 2
    infillPercent = 1 #三个外部参数的输入：文件路径、层厚（mm)，填充百分比（0-1）
    triangles = fileToTriangles(filename)#文件转化为三角面片。

    slices_ = separateSlices(triangles, layerThickness) #给三角形列表和层厚，返回各切片线段
    """
    for sl in slices_:
        print(sl)
    """
