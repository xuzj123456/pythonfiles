...
(up_buy,up_sell,down_sell,down_buy)=(90,65,35,10) #上下界
option_type = "at"# at/out，at为平值期权和虚值1期权，out为虚值1与虚值2期权
_type="call" #call/out
...
if option_type == "out" and _type=="call":
    ...
    a_name,o_name='call','out_call'
elif option_type == "at" and _type=="put":
    a_name,o_name='put','out_put'
else:
    print("_type error")
    ...
    if trade_option.empty == False:
        for at_name in trade_option['at']:
            ...
            if num>0 and ((skew!="False" and skew < per2)  or expire(at_name,date)=="T"):
                ...
                option_value=option_value+10000.0*num*at_close-15000.0*num*out_close
                change=0
    else:
        change=0
...
def expire(at_name,date):
	if np.datetime64(date) in lastdate.lasttradingdate.values and (at_name in lastdate[lastdate.lasttradingdate == date]['symbol'].values):
		expireTF="T"
	else:
		expireTF="F"
	return expireTF
...
for i,date in enumerate(etf_option_name.date.values): #date为datetime
    ...
    if i<300:
        ...
        (per1, per2, per3, per4) = (np.percentile(a[0:i], up_buy), np.percentile(a[0:i], up_sell), np.percentile(a[0:i], down_sell), np.percentile(a[0:i], down_buy))
...
def cacul_performance():
    ...
    print(total_money)
    annual_lst = [(total_money[k + 301] - total_money[k+300]) / total_money[k+300] for k in range(DAY_MAX - 1)]
