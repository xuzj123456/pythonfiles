...
class CI_AND_DI:
    ...
    def __get_best_zhibiao(self,pf_corr,pf_all,p,q):
        ...
        # pf_base = best_pf.groupby('最优TS位移位数')
        # for item in pf_base:
            # print(item)
        # raise        
        print("!"*100,"最优相关系数排序")
        ...
        list_1 = list_1_ts["指标2"].tolist()[:10]
        #按照比例获取各种分类的数据
        if len(self.type_dict_name):
    ...
    def __get_DI(self,list_df):
        ...
        for df in list_df:
            ...
            # pf_di_all.plot()
            # plt.show()
        DI_PF = pd.concat(di_list,axis=1,sort=True) 
        DI_PF.name = "DI合成指数"    
    ...
    def main_CI_DI(self,p,q,data_check_name="",best_pd_name="",original_name =""):
        ...
        if len(data_check_name) and len(best_pd_name):
            ...
            name_list.remove("滞后") #挑选指标
            pfs_all = pfs_all[ name_list] 
            
            time_item = time.strftime('%Y-%m-%d %H-%M-%S',time.localtime(time.time()))
            #pfs_all = self.zhibiao_res(pfs_all,original_name,time_item)
            
            cols = pfs_all.columns.tolist()
            print("pfs_all_222",pfs_all)
            #pfs_all = pfs_all.rolling(window=5).mean().dropna()  #移动平均2个月
            
            
            #pfs_all[cols[-1]] = pfs_all[cols[-1]].shift(-6)
            ax = pfs_all.plot(use_index=True, y=cols[:3], figsize=(10, 6),
                                secondary_y=cols[-1:], title="CI指标对比(%s)"%data_check_name)
            
            plt.savefig('%s %s.png'%(1,2),dpi=500)
            plt.show()
